            ...
            <tr class="hover:bg-slate-50">
                ...
                <td class="px-4 md:px-6 py-4 text-right">
                    <div class="flex justify-end gap-2">
                        <a href="{{ route('qr.show', $qr->id) }}" class="inline-flex items-center gap-1 px-3 py-2 rounded-lg border border-slate-200 text-slate-700 hover:border-slate-300">
                            <span class="md:hidden">👁️</span>
                            <span class="hidden md:inline">Detail</span>
                        </a>
                        <a href="{{ route('qr.edit', $qr->id) }}" class="inline-flex items-center gap-1 px-3 py-2 rounded-lg bg-indigo-600 text-white hover:bg-indigo-500">
                            <span class="md:hidden">✏️</span>
                        ...
                        <form action="{{ route('qr.destroy', $qr->id) }}" method="POST" onsubmit="return confirm('Hapus QR ini?')">
                            @csrf
                            @method('DELETE')
                            <button class="inline-flex items-center gap-1 px-3 py-2 rounded-lg bg-rose-500 text-white hover:bg-rose-600">
                                <span class="md:hidden">🗑️</span>
